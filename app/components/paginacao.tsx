interface PaginacaoProps {
  ...
  isModal?: boolean; // Adicionamos essa propriedade opcional
}
...
export default function Paginacao({
  ...
  onPaginaChange,
  children,
  isModal = false, // Por padrão, se não passar, você escolhe se é modal ou não
}: PaginacaoProps) {
  
  // 1. Renderização caso você QUEIRA que seja um Modal (com fundo escuro)
  if (isModal) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div className="bg-white rounded-xl max-w-2xl w-full p-6 shadow-xl">
          {children}
          {/* Controles de página aqui debaixo */}
...
// 2. Renderização INLINE (Sem amarras de tamanho do modal antigo)
  return (
